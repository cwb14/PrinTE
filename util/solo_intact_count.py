...
what PrinTE itself would report:
  ...
  * Intact LTR-RT  -- as in PrinTE/bin/extract_intact_LTR.py:
  ...
  * "disrupted"    -- exactly PrinTE/bin/TE_exciser_parallel.py
...
def is_disrupted(rec, window, center):
    ...
    rule 3 / TE_exciser_parallel.py condition (b) both gate on `e.supp`).
    """
    if not rec.supp:
        return False
