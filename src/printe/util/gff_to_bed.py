...
HELP_DETAIL = """
Columns in the output BED:
  1. Chromosome            from GFF column 1
  2. Start                 from GFF column 4
  3. End                   from GFF column 5
  4. Feature ID            the GFF 'Name' attribute up to the first '_', then:
                             '_FRAG' before the '#' when Integrity != 1.0
                             '#<Classification>' from the GFF
                             ';CUT_BY:<base>#<class>'   when Cut_by= is present
                             ';NESTED_IN:<base>#<class>' when Nest_in= is present
  5. TSD                   TSD_5 if set and not 'None', else TSD_3, else NA
  6. Strand                from GFF column 7
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert a TEgenomeSimulator GFF3 into PrinTE BED.",
        epilog=HELP_DETAIL,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("gff", help="Input GFF3 from TEgenomeSimulator")
    parser.add_argument("bed", help="Output PrinTE-format BED")
