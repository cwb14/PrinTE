...
def main():
    parser = argparse.ArgumentParser(
        description="Replace non-ACGT characters with T. Streams, so it handles "
                    "whole genomes without loading them. Writes FASTA to stdout.",
        epilog="examples:\n"
               "  python -m printe.util.fix_non_ATGC input.fa      > output.fa\n"
               "  python -m printe.util.fix_non_ATGC input.fa.gz   > output.fa\n"
               "  zcat input.fa.gz | python -m printe.util.fix_non_ATGC - > output.fa",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("input", help="FASTA, optionally gzipped, or - for stdin")
    in_path = parser.parse_args().input
