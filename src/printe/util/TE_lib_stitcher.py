...
###
### Caveat: pairing keys off the first "LTR" or "I" found anywhere in the header,
### so a name that already contains an I - ATCOPIA67_LTR, say - keys differently
### from its own internal and is passed through unstitched rather than failing
### loudly. merge_ltr_parts.py pairs on explicit _LTR / _I / _INT markers instead
### and handles one internal matching several LTRs; prefer it for new work.
...
# python -m printe.seq_divergence -i all.remove_dups.squeezed_headers.stitched.ltr.clean.fa -o all.remove_dups.squeezed_headers.stitched.ltr.fa.clean.tsv -t 100 --min_align 50 --max_off 20 --miu 3e-8 --blast_outfmt '6 qseqid sseqid sstart send slen qstart qend qlen length nident btop'
