...
def format_scientific(x):
    ...
    if x == 0:
        return "0"
    s = f"{x:.1e}"  # one decimal in mantissa
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Derive per-base insertion and deletion rates from a pipeline.report."
    )
    parser.add_argument("report", help="pipeline.report written by log_to_report")
