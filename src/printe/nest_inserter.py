...
from . import cutpaste_common as _cc
...
def get_tsd_length(te_class, te_superfamily):
    ...
    def _const5():
        return 5
    def _l1_rng():
        return random.randint(5, 20)   # LINE/L1
    def _cacta_rng():
        return random.randint(2, 4)    # CACTA
    def _hat_rng():
        return random.randint(5, 8)    # hAT
    def _mule_rng():
        return random.randint(8, 9)    # MULE / MuDR
    def _tourist_rng():
        return random.randint(2, 10)   # Tourist / Stowaway
    def _unk_tir_rng():
        return random.randint(2, 10)   # Unknown TIR (explicit alias)
...
def main():
    ...
    if args.fix_in is not None:
        ...
        te_ratio = {}
        try:
            with open(args.TE_ratio_file) as f:
                for line in f:
                    line = line.strip()
                    if not line:
                        continue
                    ...
                    try:
                        ...
                        # Keep the old fallback so behaviour is unchanged, but say so:
                        # a silently zero-weighted row disables that TE family for the
                        # whole run, which is easy to miss in a long log.
                        print(
                            f"warning: unparseable weight {weight_str!r} for "
                            f"{te_class}/{te_superfamily}; treating as 0",
                            file=sys.stderr,
                        )
                        weight = 0.0
    ...
    if args.fix_in is None and args.birth_rate and args.birth_file and args.TE_ratio_file:
        try:
            with open(args.birth_file) as bf:
        ...
        for _ in range(number_of_born_tes):
