...
class Stats:
    ...
    def add(self, ins, dele):
        self.count += 1
        self.min_I = ins if self.min_I is None else min(self.min_I, ins)
        self.max_I = ins if self.max_I is None else max(self.max_I, ins)
        self.min_D = dele if self.min_D is None else min(self.min_D, dele)
        self.max_D = dele if self.max_D is None else max(self.max_D, dele)
...
def main():
    ...
    for entry in sorted(scan_root.iterdir()):
        ...
        ins, dele = parse_rates(entry.name)
        if ins is None:
            continue

        all_stats.add(ins, dele)
        ...
        ok_stats.add(ins, dele)

        if ball_min <= fsize <= ball_max:
            ball_stats.add(ins, dele)
