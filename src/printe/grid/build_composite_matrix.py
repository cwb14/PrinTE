...
def parse_compare_output(stdout_text):
    """Extract numeric metrics from compare_genomes.py output."""
    metrics = {}
...
def run_compare_job(job, progress=None):
    """
    Run compare_genomes.py for one simulation directory.
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Run compare_genomes.py over all simulation dirs and build a composite matrix."
    )
    parser.add_argument(
        "--compare-script",
        default=str(Path(__file__).resolve().parent / "compare_genomes.py"),
        help="Path to compare_genomes.py (default: the copy shipped alongside this script)",
    )
    parser.add_argument(
        "--ref-tsv",
        required=True,
    ...
    parser.add_argument(
        ...
        default="wasserstein",
        help="Distribution distance metric to use in compare_genomes.py "
             "(default: wasserstein).",
    )
    parser.add_argument(
        "--alphas",
        nargs=4,
        type=float,
        # Order must match compare_genomes.py's --alphas, which unpacks as
        ...
        help="Four alpha weights forwarded to compare_genomes.py via --alphas, "
...
def main():
    ...
    # Second pass: run compare_genomes.py over all jobs, possibly in parallel
    if jobs:
    ...
    if rows and skipped_param_rows:
        ...
        print(
            "Warning: no successful compare_genomes.py runs; "
