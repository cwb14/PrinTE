...
def get_disrupted_genes(entries, promoter_boundary=0):
    ...
    for _chrom, genes in genes_by_chrom.items():
        for g in genes:
            if ";" in g.name:
...
#           if ("LTRlen" in e.feature_id) and ("_SOLO" not in e.feature_id):
    ...
    for chrom, rec in genome_records.items():
        ...
        parts = []
        prev_end = 0
        for (rstart, rend, _rlen) in events:
    ...
    for chrom, events in sorted_events_by_chrom.items():
        ...
        prefix = []
        running = 0
        for _rstart, _rend, rlen in events:
            running += rlen
...
def plot_lognormal(sigma, outname):
    ...
    plt.title(f"Log-normal Distribution (μ = {mu:.2f}, σ = {sigma:.2f})")
    plt.tight_layout()
    plt.savefig(outname)
    plt.close()
...
def plot_weighted_candidate_curve(k, L95, outname):
    ...
    plt.title(f"Weighted Candidate Selection Curve (k = {k:.2f})")
    plt.tight_layout()
    plt.savefig(outname)
    plt.close()
