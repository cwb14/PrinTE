/**********************************************************************
 * ltr_mutator.cpp - point-mutation simulator with BED-aware segments.
 *
 * Random generations follow an exponential decay distribution
 * (lambda = 1.4e-6) rather than a uniform one.
 *
 * Built on demand by the Makefile ('make ltr-mutator'), which carries the
 * Linux and macOS recipes; PrinTE.sh builds it on first use if it is absent.
 * Needs C++17 and OpenMP.
 *********************************************************************/
...
void print_usage() {
    std::cerr <<
        "Usage: ltr_mutator -fasta <in.fa> -rate <u> -generations <G> "
