...
def _run(cmd: List[str], timeout_s: int) -> Optional[str]:
...
def _grep_config(cfg: str, key: str) -> Optional[str]:
...
def _time_to_minutes(s: Optional[str]) -> Tuple[Optional[int], bool]:
...
def classify_partition(oversub: Optional[str], exclusive_field: Optional[str],
                       select_type: Optional[str]) -> Tuple[str, str]:
...
def _detect_default_partition(timeout_s: int) -> Optional[str]:
...
def probe_cluster(requested_partition: Optional[str], *, timeout_s: int = 8,
