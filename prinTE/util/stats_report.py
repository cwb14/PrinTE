...
def process_bed_file(bed_file):
    """
    Process one BED file and return overall counts and TE breakdowns.
    Returns:
    ...
    for record in lines:
        ...
        overall_counts[record['category']] += 1  # Now this won't fail
    ...
    return overall_counts, intact_TE_counts, frag_TE_counts

def merge_dicts(dict_list, keys):
    """
    Given a list of dictionaries and a set of all keys, return a list of counts
...
def main():
    parser = argparse.ArgumentParser(
        description="Classify BED entries from multiple files into gene and TE categories and produce summary stats."
    )
    parser.add_argument("--bed", nargs="+", required=True, help="Input BED file(s)")
    parser.add_argument("--out_prefix", required=True, help="Output prefix for stats files")
