...
def classify_entries(entries):
    ...
    by lines that share identical feature_ID, strand and TSD.
...
def simulate_excision(genome_records, entries, nest_groups, removals, soloLTR_freq):
    ...
    for gid, group in nest_groups.items():
        if gid in nest_middle_removed:
            ...
            new_name = group[0].feature_id  # no supplemental info added here
...
def main():
    parser = argparse.ArgumentParser(
        description="Simulate TE excisions from a genome based on a BED file.\n"
                    ...
                    "Additionally, disrupted genes now contribute based on a weight. The script looks for a file "
                    "'./gene_selection.tsv' with two columns: geneID and disrupted_gene_weight. If it does not exist, "
                    "it will be created using synthetic values drawn from a log-normal distribution (with mode=1).\n\n"
                    "NEW: Eligible excision events are selected using a weighted probability distribution based on their length.\n"
                    "     Use --k to set the decay rate (e.g., --k 0.5, --k 2, etc.).\n"
