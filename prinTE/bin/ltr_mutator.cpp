...
#include <chrono>
#include <mutex>
#include <cmath>        // Added for exp function
#include <omp.h>

// Function to display usage information
void print_usage() {
    std::cout << "Usage:\n"
              << "  mutator -fasta <input_fasta> -rate <mutation_rate> -generations <num_generations> "
              << "-mode <0|1> -threads <num_threads> -seed <seed> -out_prefix <prefix>\n"
              << "\nArguments:\n"
              ...
              << "  -seed         Seed for random number generation (e.g., 42).\n"
              << "  -out_prefix   Prefix for output files (e.g., gen_1000).\n";
}
...
bool parse_arguments(int argc, char* argv[],
    ...
    // Now expecting 7 parameters (flag/value pairs) plus program name: 15 arguments in total
    if (argc != 15) {
        print_usage();
        return false;
    }
    ...
    for (int i = 1; i < argc; i += 2) {
        ...
        if (arg == "-fasta") {
            ...
            try {
                ...
                std::cerr << "Error: Invalid seed provided.\n";
                return false;
            }
        } else if (arg == "-out_prefix") {
            out_prefix = argv[i + 1];
        } else {
...
int main(int argc, char* argv[]) {
    ...
    int mode;
    int threads;
    int seed;                   // New parameter for reproducibility
    std::string out_prefix;     // New parameter for output file prefix

    // Parse command-line arguments
    if (!parse_arguments(argc, argv, fasta_path, mutation_rate, generations, mode, threads, seed, out_prefix)) {
        return EXIT_FAILURE;
    }
    ...
    // Calculate genome size: sum of all sequence lengths
    ...
    {
        // Each thread gets its own RNG, seeded deterministically using the provided seed and thread number
        std::mt19937 rng;
        ...
        for (size_t idx = 0; idx < total_sequences; ++idx) {
            ...
            std::string sequence = sequences[idx].second; // Copy for mutation
            ...
            if (total_mutations > 0) {
                ...
                for (int mut = 0; mut < total_mutations; ++mut) {
                    ...
                    // Recurrent mutations are allowed.
                }
            }
    ...
    // Define output FASTA file name using the specified out_prefix
    std::string output_fasta = out_prefix + ".fa";
    ...
    // If mode is 0, write the supplementary txt file using out_prefix
    if (mode == 0) {
        std::string output_txt = out_prefix + ".txt";
        ...
        // Calculate metrics
        double lambda = static_cast<double>(total_mutations_sum) / static_cast<double>(genome_size);
        double U = static_cast<double>(genome_size) * (1.0 - std::exp(-lambda));
        double recurrent_mutations = static_cast<double>(total_mutations_sum) - U;
        double mutations_per_site = U / static_cast<double>(genome_size);
        double mutations_per_site_times_two = mutations_per_site * 2.0;

        // Write metrics to the supplementary file
        txtfile << "Genome size: " << genome_size << "\n";
        txtfile << "Total number of mutations: " << total_mutations_sum << "\n";
        txtfile << "Estimated number of recurrent mutations: " << recurrent_mutations << "\n";
        txtfile << "Estimated number of mutations per site: " << mutations_per_site << "\n";
        txtfile << "Estimated number of mutations per site * 2: " << mutations_per_site_times_two << "\n";
